...
export function isPlayerJoined(arg: any): arg is PlayerJoined {
  return arg && arg.kind === "PlayerJoined" && isNumber(arg.id) && isNumber(arg.x) && isNumber(arg.y) && isString(arg.style);
}
...
export interface ImMoving {
  kind: "ImMoving";
  start: boolean;
  direction: Direction;
}

export function isImMoving(arg: any): arg is ImMoving {
  return arg && arg.kind === "ImMoving" && isBoolean(arg.start) && isDirection(arg.direction);
}
